...
await p.locator('div:text-is("Kl.")').click(); // depart exactly at 12:00
await p.locator('input[type=time]').fill('12:00');
...
const ACC = '#0A9C93'; // light theme accent (app default)
const selBefore = badgesBefore.find(b => b.includes('background:' + ACC));
const selAfter = badgesAfter.find(b => b.includes('background:' + ACC));
